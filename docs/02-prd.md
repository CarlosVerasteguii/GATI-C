...
Stack de Estilos (6.6): Tailwind CSS + shadcn/ui como stack oficial de componentes y estilos. Theming por CSS variables con `next-themes`. Se eliminan referencias a frameworks base o a un stack híbrido.
Modo Oscuro (6.5): Soportado desde el MVP mediante `next-themes` (estrategia `class`). Default: tema claro; toggle disponible en el header.
Colores de Estado (6.4): Uso de paleta semántica para estados de activos y UI. Mapeo base — Disponible: Verde; Asignado: Púrpura/Índigo; Prestado: Amarillo/Ámbar; Pendiente de Retiro: Naranja; Retirado: Rojo; Info: Azul; Éxito: Verde; Error: Rojo; Advertencia: Naranja. La implementación usa clases semánticas (`bg-status-*-bg`, `text-status-*-text`) provistas por Tailwind.
