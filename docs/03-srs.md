...
2. Architecture Pattern
	...
	2.2. Frontend (Client): Next.js App Router con React Server Components (RSC) por defecto. Enfoque híbrido: RSC para contenido de solo lectura/render-first; Client Components solo cuando haya interactividad.
...
2.	The corresponding action calls the frontend API layer (endpoints) which performs the request. In client components, data flows through SWR hooks; direct fetch/axios in UI is prohibited.
...
o	Data Fetching: SWR (client standard).
	Backend:
...
o	User (id, name, email, password_hash, role, trusted_ip, deleted_at)
...
Appendix A — Frontend Rendering & Data‑Fetching Policy (v2.3)

- Rendering (App Router): Server Components by default. Use "use client" only for interactivity (hooks, state, events, browser APIs). Prefer streaming and skeletons over spinners for first paint.
- RSC/SSR data: Use Next.js fetch with caching controls: revalidate, tags, or cache: 'no-store' according to data volatility/sensitivity.
- Client data: Exclusive via SWR hooks defined under @/lib/api/hooks. Direct fetch/axios in UI components is not allowed; requests go through the API layer.
- API layer: Located at @/lib/api/ with http.ts, schemas.ts, endpoints/, and hooks/ to centralize contracts, parsing (Zod), and caching keys.
- Acceptance:
  - No page.tsx/layout.tsx marked client without actual interactivity.
  - No direct fetch/axios in client components.
  - Pages define appropriate caching/invalidation (revalidate/tags).
